import React from "react";
import { motion, Variants } from "framer-motion";

interface LogoIconProps {
  className?: string;
}

const LogoIcon: React.FC<LogoIconProps> = ({ className }) => {
  const drawVariants: Variants = {
    hidden: { 
      pathLength: 0, 
      fillOpacity: 0,
      strokeOpacity: 1 
    },
    visible: {
      pathLength: 1,
      fillOpacity: 1,
      strokeOpacity: 0,
      transition: {
        pathLength: { duration: 2, ease: "easeInOut" },
        fillOpacity: { duration: 0.8, delay: 1.5, ease: "easeOut" },
        strokeOpacity: { duration: 0.8, delay: 1.5, ease: "easeOut" }
      }
    }
  };

  return (
    <motion.svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="327.618 276.758 357.963 459.224"
      className={className}
      initial="hidden"
      animate="visible"
    >
      <g 
        transform="translate(0.000000,1024.000000) scale(0.100000,-0.100000)" 
        fill="currentColor"
        stroke="currentColor" 
        strokeWidth="40" 
        strokeLinecap="round"
        strokeLinejoin="round"
      >
        <motion.path 
          variants={drawVariants}
          d="M3600 5126 c0 -1774 1 -1924 16 -1930 25 -9 1835 -6 1988 4 74 4 167 17 214 29 327 84 622 329 741 615 62 150 74 213 74 401 1 157 -1 177 -26 258 -51 166 -127 290 -252 412 -77 76 -193 155 -277 190 l-38 15 0 -600 0 -600 -454 0 -455 0 -438 -206 c-241 -113 -573 -270 -738 -349 -316 -150 -358 -168 -339 -144 6 8 51 66 100 129 48 63 146 189 216 280 312 403 419 550 411 563 -4 7 -97 126 -205 263 -581 736 -529 665 -510 689 9 13 46 60 81 106 36 46 133 170 216 274 124 155 222 281 416 531 9 11 -58 101 -420 574 -83 107 -186 240 -229 295 -44 55 -77 101 -73 103 8 3 832 -385 1254 -590 l268 -130 467 2 c257 1 474 2 483 1 14 -1 227 -231 345 -373 27 -32 33 -35 39 -20 14 32 18 252 5 328 -32 201 -130 386 -281 529 -128 121 -254 191 -435 242 l-99 28 -1032 3 -1033 3 0 -1925z" 
        />
        <motion.path 
          variants={drawVariants}
          d="M5770 5883 l-635 -3 -220 -191 c-121 -105 -305 -266 -409 -358 l-189 -166 129 -8 c71 -4 374 -6 674 -4 477 3 554 6 620 21 337 80 613 328 709 640 11 37 21 70 21 72 0 2 -15 3 -32 2 -18 -2 -319 -4 -668 -5z" 
        />
        <motion.path 
          variants={drawVariants}
          d="M4335 5072 c43 -40 353 -321 569 -516 l228 -206 295 0 c192 0 294 3 291 10 -1 5 -165 172 -363 370 l-360 360 -340 0 c-329 0 -339 -1 -320 -18z" 
        />
      </g>
    </motion.svg>
  );
};

export default LogoIcon;